...
Из двух уравнений протокол выводит `2·sigma=1`, затем `sigma=0.5`, подставляет найденную координату в первое уравнение и получает `mirrorSigma=0.5`. Это доказывает только условное следствие для **данной явно заданной пары**.

## Регрессионные проверки

| Тест | Контракт |
|---|---|
| `RIEMANN01` | Строит доказательство `sigma=0.5`; проверяет реальные имена параметров в `StringBuilder`, все четыре линейных шага и отсутствие ложной декларации о доказательстве гипотезы Римана. |
