...
internal static class ExampleCatalog
{
    ...
    [
        ...
        new("L59", "Максимум выражений", "x => max(x, 5)"),
        new("L60", "Ограничение диапазона", "x => clamp(x, -1, 1)"),
        new("L61", "Гиперболический косинус", "x => cosh(x)"),
        new("L62", "Гиперболический тангенс", "x => tanh(x)"),
        new("L63", "Десятичный логарифм", "x => log10(abs(x) + 1)"),
        new("L64", "Знак выражения", "x => sign(x)"),
        new("L65", "Остаток от деления", "x => mod(x, 2)"),
        new("L66", "Степенная функция", "x => pow(x, 3)"),
    ];
}
