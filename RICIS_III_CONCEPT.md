...
### Строгий протокол обращения к классике

Классический fallback является разрешённым наследованием семантики, но не автоматическим основанием для рассуждения. Перед любым обращением к классической математике в анализе, proof trace, QA-выводе или проектировании функции необходимо выполнить следующие шаги:

1. Проверить, что для рассматриваемой операции действительно отсутствует явное правило RICIS III, включая L0/L1, SP1–SP4, O(1)-bridges, A-rules, type consistency и structural algebra.
2. Явно сообщить, какая операция остаётся не покрыта RICIS и какой классический результат предлагается применить.
3. **Запросить разрешение пользователя до использования классического результата в рассуждении или выводе.**
4. До получения разрешения не подменять RICIS-сущность числом, не применять классическое domain reasoning и не объявлять classical result частью доказательства.

Если операция уже покрыта RICIS, разрешение на fallback не требуется и классический результат не может иметь приоритета. Например, для одинакового deferred expression действует `Log(F)/Log(F) = 1` по L1 до вычисления `Log`, независимо от классической области определения. Этот протокол обязателен также для QA-аудита и анализа недостающих функций.
...
Для coupled raw-формы bridge распознаётся до общего A1-индексирования reciprocal: `F·0` сначала образует `0_F`, а `(1/F)` сохраняет denominator payload как `∞_F`; поэтому `(F·0)·(1/F) → 0_F·∞_F → F²`. Общий A1 result `∞_1` для изолированного `1/F` не имеет права стереть эту связанную A6-структуру.
