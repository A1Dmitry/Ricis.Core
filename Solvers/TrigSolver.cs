...
public static class TrigSolver
{
    public static (ParameterExpression, double)? Solve(this Expression expr)
    {
        ...
        if (paramExpr == null || Math.Abs(a) < double.Epsilon)
        {
            return null;
        }
    ...
    public static bool TryExtractLinear(Expression expr, out ParameterExpression param, out double a, out double b)
    {
        ...
        if (expr is BinaryExpression add && (add.NodeType == ExpressionType.Add || add.NodeType == ExpressionType.Subtract))
        {
            ...
            var sign = add.NodeType == ExpressionType.Subtract ? -1.0 : 1.0;
            ...
            if (left is BinaryExpression leftMul && leftMul.NodeType == ExpressionType.Multiply && right is ConstantExpression rc)
            {
                ...
                {
                    if (!TryConst(cL, out a))
                    {
                        return false;
                    }

                    param = pR;
                    if (!TryConst(rc, out var rcVal))
                    {
                        return false;
                    }
                ...
                {
                    if (!TryConst(cR2, out a))
                    {
                        return false;
                    }

                    param = pL;
                    if (!TryConst(rc, out var rcVal2))
                    {
                        return false;
                    }
            ...
            if (right is BinaryExpression rightMul && rightMul.NodeType == ExpressionType.Multiply && left is ConstantExpression lc)
            {
                ...
                {
                    if (!TryConst(cL2, out a))
                    {
                        return false;
                    }

                    param = pR2;
                    if (!TryConst(lc, out var lcVal))
                    {
                        return false;
                    }

                    b = lcVal; // add or subtract handled by sign earlier
                    if (add.NodeType == ExpressionType.Subtract)
                    {
                        b = -b;
                    }
                ...
                {
                    if (!TryConst(cR3, out a))
                    {
                        return false;
                    }

                    param = pL3;
                    if (!TryConst(lc, out var lcVal2))
                    {
                        return false;
                    }
