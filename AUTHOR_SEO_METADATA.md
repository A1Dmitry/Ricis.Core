...
Переменная должна быть **фактически использована** в теле лямбды, чтобы компилятор создал захват. Параметр лямбды с именем `about` не активирует метаданные. У лямбды без захваченного `about` вывод остаётся прежним.
