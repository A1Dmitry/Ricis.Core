...
public static class RicisPiecewiseSurfaceExample
{
        ...
        ICollection<RicisPhaseTraceStep> trace = null)
    {
        ...
        RecordStep(log, trace, "RICIS_PIECEWISE_X_STRIP", "eq2: собирается полоса 0 <= x <= 5.", lowerX, xStrip, "eq2");
        var yStrip = Expression.AndAlso(lowerY, upperY);
        RecordStep(log, trace, "RICIS_PIECEWISE_Y_STRIP", "g: собирается полоса 1 <= y <= 5.", lowerY, yStrip, "g");
        var parabola = Expression.Divide(Expression.Multiply(x, x), five);
        var aboveParabola = Expression.GreaterThan(y, parabola);
        RecordStep(log, trace, "RICIS_PIECEWISE_PARABOLA", "b: строится строгая граница y > x^2 / 5.", y, aboveParabola, "b");
        var domain = Expression.AndAlso(xStrip, Expression.AndAlso(yStrip, aboveParabola));
        RecordStep(log, trace, "RICIS_PIECEWISE_DOMAIN", "Пересекаются eq2, g и b: формируется область определения.", xStrip, domain, "SP2 / domain intersection");
        var product = Expression.Convert(Expression.Multiply(x, y), typeof(double?));
        RecordStep(log, trace, "RICIS_PIECEWISE_VALUE", "Внутри области строится ветвь a(x,y) = x*y.", Expression.Multiply(x, y), product, "A1 / value branch");
        var nullValue = Expression.Constant(null, typeof(double?));
        var body = Expression.Condition(domain, product, nullValue);
        RecordStep(log, trace, "RICIS_PIECEWISE_CONDITIONAL", "Ветви объединяются: внутри x*y, вне области null.", product, body, "A4 / conditional branch");
