...
| `collapsed_type_violates_id03` | Коллапсирующая функция `ℚ→Unit` не может быть верным идентификатором координаты; совпадение значения на `0` и `1` отвергается. |

Эта модель проверяет, что заявленная аксиоматическая цепочка замыкается логически. Она не содержит `sorry`; поля структуры являются **явно переданным нормативным фундаментом**, а не скрытыми допущениями. Теорема `collapsed_type_violates_id03` отдельно исключает обход ID-03 через функцию, которая возвращает одно и то же значение для разных координат.
