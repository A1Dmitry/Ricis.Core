...
internal static class RicisStressSuite
{
    ...
    [
        ...
        ("S05: sin(x)/cos(x) сохраняет parent индекс Sin(x)", S05SimpleTrig),
        ("S06: sin(x)/x остаётся отношением отложенных F/G", S06Sinc),
        ("S07: sin(2x)/cos(2x) сохраняет parent индекс Sin(2x) и все ключи", S07CompositeTrig),
    ...
    private static void S07CompositeTrig()
    {
        ...
        {
            throw new InvalidOperationException($"S07: ожидалось ∞_{{Sin(2x)}} с ключами, получено {output}.");
        }

        Require(infinity.Numerator.AreEqual(f),
            $"S07 должен сохранить parent индекс Sin(2x), получено {infinity.Numerator}.");
        Require(infinity.Roots.Count > 1 &&
                infinity.Roots.Any(root => root.Value < 0d) &&
                infinity.Roots.Any(root => root.Value > 0d),
            "S07 должен сохранить несколько сертифицированных ключей по обе стороны от нуля.");
    }
