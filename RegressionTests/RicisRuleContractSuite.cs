...
internal static class RicisRuleContractSuite
{
    ...
    [
        ...
        ("RC26: SP2/A1 — 1/(1−2/x) сохраняет parent индекс x при x=2", NestedRatioPole),
        ("RC27: A1 — 1/(1−tan x) не получает полюса tan", CertifiedTrigRoots),
        ("RC28: SP4 — близкие корни не сливаются", CloseRootsRemainDistinct),
        ("RC29: A1 — x²/(a·x²+a·b+c) сохраняет parent индекс x² для обоих ключей", ParameterizedQuadraticPole),
        ...
        ("RC32: A1 — x/(x²−1) сохраняет один parent индекс x и оба ключа", DistinctKeyIndicesRemainAssociated),
    ...
    private static void NestedRatioPole()
    {
        ...
        AssertInfinityRoots(RicisPhasePipeline.Simplify(Expression.Lambda<Func<double, double>>(source, x)), x, [2],
            "SP2/A1 для вложенной дроби: parent индекс после SP2 — x");
    }
    ...
    private static void ParameterizedQuadraticPole()
    {
        ...
        AssertInfinityRoots(RicisPhasePipeline.Simplify(Expression.Lambda<Func<double, double>>(source, x)), xSquared, [-1, 1],
            "A1 для a=1, b=1, c=−2: parent индекс x² сохраняется для обоих ключей");
    }
    ...
    private static void DistinctKeyIndicesRemainAssociated()
    {
        ...
        AssertInfinityRoots(
            RicisPhasePipeline.Simplify(Expression.Lambda<Func<double, double>>(source, x)),
            x,
            [-1, 1],
            "A1/SP4 для x/(x²−1): parent индекс x сохраняется для каждого ключа");
    }
